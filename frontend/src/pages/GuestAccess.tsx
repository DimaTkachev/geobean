...
const GuestAccess: React.FC = () => {
  ...
  const [qrData, setQrData] = useState<{ shareUrl: string; qrBase64: string; guestUrl: string } | null>(null);
  ...
  const handleGenerateQr = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/shops/${currentShop.shopID}/generate-qr`, {
        method: 'POST',
  ...
  const handleSidebarToggle = () => {
    ...
    localStorage.setItem('guestAccessSidebarExpanded', JSON.stringify(newExpanded));
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#f5e0d1' }}>
      ...
      >
        ...
        >
          <span style={{ fontSize: 24, color: accent }}>{isSidebarExpanded ? '←' : '→'}</span>
        </button>
            ...
            >
              <div
                style={{
                  ...
                  height: 36,
                  borderRadius: '50%',
                  background: shop.theme === 'beige' ? accent : shop.theme === 'purple' ? purple : blue,
              ...
              {isSidebarExpanded && (
                <button
                  ...
                  style={{ background: 'none', border: 'none', marginLeft: 'auto', color: accent, cursor: 'pointer', fontSize: 14 }}
                  title="Редактировать"
                >
                  Ред.
                </button>
      ...
      {/* Main content */}
      <main style={{ flex: 1, padding: '48px 0 0 0', display: 'flex', flexDirection: 'column', alignItems: 'center', background: '#f5e0d1' }}>
        <h1 style={{ fontSize: 38, fontWeight: 700, marginBottom: 18, color: '#3c1f0c', textAlign: 'center' }}>Гостевой доступ</h1>
        <p style={{ marginBottom: 32, maxWidth: 440, textAlign: 'center', color: '#3c1f0c', fontSize: 18 }}>
          Упростите выбор для гостей. QR-код откроет карту с тем, что сейчас есть в наличии – удобно и наглядно.
        </p>
        {error && <div style={{ color: 'red', marginBottom: 16 }}>{error}</div>}
        {qrData ? (
          <>
            <img src={qrData.qrBase64} alt="QR code" style={{ width: 220, height: 220, marginBottom: 18, borderRadius: 8, boxShadow: '0 2px 12px rgba(0,0,0,0.10)' }} />
            <div style={{ marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="text" value={qrData.guestUrl} readOnly style={{ width: 340, padding: 10, fontSize: 17, borderRadius: 6, border: '1px solid #d4a88c', background: '#fff', color: '#3c1f0c' }} />
              <button onClick={() => navigator.clipboard.writeText(qrData.guestUrl)} style={{ background: accent, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 12px', fontSize: 18, cursor: 'pointer' }}>📋</button>
            </div>
            <button onClick={handleGenerateQr} disabled={loading} style={{ background: accent, color: '#fff', padding: '14px 38px', fontSize: 18, border: 'none', borderRadius: 6, fontWeight: 600 }}>
              {loading ? 'Генерируется...' : 'Сгенерировать новый QR-код'}
            </button>
          </>
        ) : (
          <button onClick={handleGenerateQr} disabled={loading || !currentShop} style={{ background: accent, color: '#fff', padding: '14px 38px', fontSize: 18, border: 'none', borderRadius: 6, fontWeight: 600 }}>
