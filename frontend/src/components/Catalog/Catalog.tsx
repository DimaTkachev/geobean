...
export const Catalog: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (filters.tasteTags.length > 0) {
      filtered = filtered.filter(lot => lot.tasteTags.some(tag => filters.tasteTags.includes(tag)));
    }

    if (filters.suppliers.length > 0) {
      filtered = filtered.filter(lot => filters.suppliers.includes(lot.supplier));
    }
    ...
    <div className={`${styles.container} ${isSidebarExpanded ? styles.sidebarExpanded : ''}`}>
      ...
      >
        <button
          ...
          onClick={(e) => { e.stopPropagation(); setIsSidebarExpanded(exp => !exp); }}
          aria-label={isSidebarExpanded ? 'Свернуть' : 'Развернуть'}
        >
          <span style={{ fontSize: 24 }}>{isSidebarExpanded ? '←' : '→'}</span>
        </button>

        <div style={{ marginBottom: isSidebarExpanded ? 24 : 0, overflow: 'hidden' }}>
          {isSidebarExpanded && (
            <div style={{ marginBottom: 12 }}>
              {shops.map((shop) => (
                <div
                  ...
                  onClick={(e) => { e.stopPropagation(); setCurrentShop(shop); }}
                >
                  <div
                    style={{
                      width: 36,
           ...
           <div className={styles.filters}>
             ...
             <div className={styles.searchContainer}>
              ...
              <div className={styles.filterSection}>
                <h4 className={styles.filterSubtitle}>Тип обжарки:</h4>
              ...
              <div className={styles.filterSection}>
                ...
                <div className={styles.filterOptions}>
                  {availableFilters.suppliers.map(supplier => (
                    <label key={supplier} className={styles.filterOption}>
                      <input
                        type='checkbox'
      ...
      <div style={{ flexGrow: 1, padding: '20px', overflowY: 'auto' }}>
        <h2 style={{ color: '#3c1f0c', marginBottom: '20px' }}>Каталог зерен</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
          {filteredCoffeeLots.map(lot => (
            <div
              ...
              onMouseEnter={() => setHoveredCardId(lot.coffeeLotID)}
            ...
            >
              {lot.imageFilename && (
                <img
                  ...
                  alt={lot.name}
                  style={{ width: '100%', height: '150px', objectFit: 'contain', borderRadius: '4px', marginBottom: '10px', cursor: 'pointer' }}
              ...
              <h4
                style={{ marginBottom: '5px', color: '#3c1f0c', cursor: 'pointer' }}
                onClick={() => navigate(`/coffee-lots/${lot.coffeeLotID}`)}
                onKeyDown={e => { if (e.key === 'Enter') navigate(`/coffee-lots/${lot.coffeeLotID}`); }}
                tabIndex={0}
