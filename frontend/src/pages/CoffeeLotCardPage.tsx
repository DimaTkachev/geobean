...
export const CoffeeLotCardPage: React.FC = () => {
  ...
  const handleQuantity = async (change: number) => {
    ...
    try {
      const response = await fetch(`/api/shops/${currentShop.shopID}/inventory/${lot.lotID}`, {
        method: 'PATCH',
        headers: { 
  ...
  const handleSidebarToggle = () => {
    ...
    localStorage.setItem('coffeeLotSidebarExpanded', JSON.stringify(newExpanded));
  };
          ...
          <div className={styles.imageSection}>
            ...
            {isAuthenticated && (
              <div className={styles.inventoryControls}>
                ...
                <span className={styles.quantity}>{inventory ?? 0} шт</span>
                <button className={styles.quantityBtn} onClick={() => handleQuantity(1)}>+</button>
              </div>
            )}
          </div>
          <div className={styles.infoSection}>
            ...
            <div className={styles.supplierSection}>
              Поставщик: <span 
                ...
                style={{ cursor: lot.supplierLink ? 'pointer' : 'default' }}
              >
                {lot.supplier}
              </span>
            </div>
