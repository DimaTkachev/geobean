...
const GuestInventory: React.FC = () => {
    ...
    const [shop, setShop] = useState<{ name: string; theme: string } | null>(
        null
    );
    ...
    useEffect(() => {
        ...
        fetch(`/api/shops/guest-inventory/${shareUrl}`)
            .then((res) => {
                if (!res.ok)
                    throw new Error('Магазин не найден или недоступен');
                return res.json();
            })
            .then(async (data) => {
                setShop(data.shop);
                ...
                const lotDetails = await Promise.all(
                    (data.inventory || []).map(
                        async (item: { lotID: number }) => {
                            const lot = await fetchApi<any>(
                                `/api/coffee-lots/${item.lotID}`
                            );
                            // Fetch marker for coordinates
                            const allMarkers: any[] =
                                await fetchApi<any[]>('/api/map/all');
    ...
    useEffect(() => {
        setAvailableFilters({
            continents: Array.from(
                new Set(lots.map((lot) => lot.continent).filter(Boolean))
            ),
            roastingTypes: Array.from(
                new Set(lots.map((lot) => lot.roasting).filter(Boolean))
            ),
            processingMethods: Array.from(
                new Set(lots.map((lot) => lot.processingMethod).filter(Boolean))
            ),
            tasteTags: Array.from(
                new Set(lots.flatMap((lot) => lot.tasteTags || []))
            ),
        });
    }, [lots]);
        ...
        <div className={styles.container}>
            ...
            <div className={styles.mapContainer}>
                ...
                {hoveredLotID !== null &&
                    popupPosition &&
                    (() => {
                        const lot = filteredLots.find(
                            (l) => l.lotID === hoveredLotID
                        );
                        if (!lot) return null;
                        return (
                            <div
